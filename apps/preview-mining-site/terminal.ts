 ...
 *
 * Everything else here — the world, the site model, the renderer, the option
 * parser, the stats report — is either a pure function of its arguments or an
 * `Effect` over an explicit store. Node's stdio lives behind this file so the
 * boundary is visible rather than sprinkled.
 *
 * Note what is NOT here: any read of a clock. The site advances one FRAME in
 ...
 * taken. That is not a limitation being worked around. `StageRegistration.run`
 * takes `dt` as an ARGUMENT precisely so a frame can be advanced at any speed
 * (docs/testing.md §5), and a preview that drove the falling-block cascade off a
 * wall clock would show a different cascade on a faster machine — which is the
 * one property `test/rules.test.ts` calls "what makes a scenario test an
 * oracle".
 *
 * Adapted from mx-redstone's `apps/preview-circuit-board/terminal.ts`, itself
 * adapted from mc-worldgen's. The copies are deliberate; see `ansi.ts`.
 */
