...
| `arena` | **本物。Mob は 1 体（クリーパー）** | `domain/mob/` の 4 本と、それが到達する `domain/death-cause.ts`。スポーン判定 → 導火線 → 爆風 → 死因 → ドロップを実際に叩く。**状態はこの画面が持つ**（mc-sim の役） |

**アリーナは長いあいだ「Mob は存在しない」と 1 行目に書いていた。** 今は 1 体いる。
変わっていないのは**残りを列挙する習慣**のほうで、実装済みの節（4 行）より
missing の節（8 行）のほうが長い。それが正直な比率である —— plan.md §3.11 は 4 種の Mob 挙動を挙げ、
ここにあるのは 1 種で、移植可能な Mob テストは 61 ファイル中 6 本しか来ていない。
...
- **ブロックのドロップテーブルが無い。** `Broken.yielded` は「そこにあったブロック」そのものであり、
  ルートテーブルを通っていない。石を掘ると石が出る。**Mob のドロップは本物である**
  （`domain/mob/mob-drop.ts`。kernel の語彙で `gunpowder`）——
  kernel の `BlockDrop` が運び出す `affectedByFortune` を適用する側が、まだブロック側に無いだけである。
- **流体は伝播しない。** `gameplay:fluids` はフロンティアを回すだけで、伝播ルールは存在しない。
  水も溶岩も**落下ブロックの受け皿**としてだけ登場する（`lava-pit` シナリオ）。
- **Mob は 1 体だけ。** エンダーマン / シュルカー / ドラゴンは未着手であり、経路探索も無い
  （クリーパーの距離は矢印キーである）。爆発のクレーターも無い —— ブロックを書く別のルールであり、
