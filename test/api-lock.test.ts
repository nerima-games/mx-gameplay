...
describe('declaration entry point mapping', () => {
  it.effect('maps entry points relative to TypeScript\'s common source root', () =>
    Effect.sync(() => {
      expect(declarationPathFor('src/index.ts', '/virtual-dts', '/repository/src', '/repository')).toBe(
        '/virtual-dts/index.d.ts',
      )
    }),
  )
})
...
describe('per-repository configuration', () => {
  it.effect('points at the shipped-source project, not the editor-wide one', () =>
    Effect.sync(() => {
      ...
      expect(REPOSITORY_POLICY.entryPoints).toStrictEqual(['src/index.ts'])
