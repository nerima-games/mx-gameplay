...
### 5-3-1. `RailShape` の値集合は表の後で広がった —— コーナーが曲がらなかったので

上の表は `RailShape` を「語彙」とだけ書いているが、値集合そのものが一度誤っていた。
`resolveRailShape` は当初、直交する近傍が 2 つあれば向きを問わず単一の `'curve'` を返しており、
`projectMinecartVelocity` はその向きを知る手段が無いまま「入ってきた速度の支配的な軸を
そのまま継続する」しかできなかった。直線区間はハンドオフ時に垂直成分を必ずゼロにするので、
カートは**常に軸に沿った状態でしかコーナーに渡らない** —— つまり「継続する」しかできない
実装は、実際に起こる唯一の入力で**曲がらず直進してレールの外に出る**。

**参照実装にこの2つの記号のオラクルは無い。** `test/rail.test.ts` の他の全 `describe` ブロックは
`<reference-impl>` への `file:line` を引いているが、`projectMinecartVelocity` のブロックには
一度も無く、導入コミットのメッセージも引用の無い 1 行だけだった。空白を埋めたのは参照実装では
なく、このゲーム自身がプレイヤーに見せているレールの形である —— 直線 2 + 上り坂 4 + 曲線 4
（北東 / 北西 / 南東 / 南西）の 10 種で、単一の `'curve'` はその中に無い。**`mc-kernel` は
これを裏付けない**（`block-property-data.ts:43` の `RAIL_KINDS` は `'none' | 'normal' |
'powered'` のみで、向きを持たない）ので、この 4 値は保存された状態を読んだのではなく、
`resolveRailShape` が既に計算していた近傍の真偽値（`north` / `south` / `east` / `west`）を
捨てずに使うだけで導出した。3 本以上の枝が立つ T字路・十字路には対応する単一の連結対が無いため、
そこだけは旧来の undirected `'curve'` と `towardX` の継続動作を残している。

設計の全体は `domain/vehicle/rail-shape.ts` の `RailShape` と `projectMinecartVelocity` の
doc comment にあり、証拠は `test/vehicle-rail-simulation.test.ts` の閉じた矩形周回テストである
（本節はその要約で、争える判断はしていない）。
