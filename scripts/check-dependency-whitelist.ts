 ...
 *
 * ---------------------------------------------------------------------------
 * mx-gameplay is an EXPERIENCE MODULE (plan.md §2.2, §3.11): a verb, not a
 * noun. It owns rules — mining, placement, item use, mob AI, drops, fluid
 * propagation, vehicles, portals, day/night and weather — and owns no state:
 * every piece of state it touches lives in mc-sim or mc-worldgen.
 *
 * Its parents are mc-sim, mc-worldgen and mc-audio (plus mc-kernel, which is
 * universal). Two consequences are worth spelling out because this repository
 * is the one most likely to violate them:
 *
 *   1. NO TRANSITIVE CLOSURE. mc-sim depends on mc-physics; mx-gameplay does
 *      NOT. An `import ... from '@nerima-games/mc-physics'` here is reported as
 *      `transitive-import` with the path that explains it. If gameplay needs a
 *      physics answer it asks mc-sim, which is the repository that owns the
 *      question.
 *   2. NO EDGE TO ANOTHER EXPERIENCE MODULE. mx-redstone, mx-ui and
 *      mx-multiplayer are siblings, not parents. Ordering relative to them is
 *      expressed as a StageId string in `after`, resolved by mc-compose — a
 *      string is not an import and creates no edge.
 *
 * mc-playground-kit powers this repository's three previews and is therefore a
 * devDependency ONLY. plan.md §2.3-2: the runtime input service lives in
 * mc-render, and kit is not shipped, so a runtime dependency on kit would
 * delete input handling from the released game. Rule 6 makes that a hard
 * failure in two places — `dependencies` in package.json, and any import of
 * kit from shipped source.
 * ---------------------------------------------------------------------------
 */
...
export const SCAN_ROOTS: ReadonlyArray<string> = ['apps', 'src/index.ts', 'src/domain', 'src/stages', 'scripts', 'test']
...
/**
 * Shipped source is `src/index.ts` and everything under `src/domain/` and
 * `src/stages/`.
 *
...
export const isToolingOrTestPath = (relativePath: string): boolean =>
  !(
    ...
    relativePath.startsWith('src/stages/')
  )
