 ...
 *   root. This repository exports exactly `.` from `src/index.ts`.
 ...
 */
export type EmittedDeclarations = {
  readonly files: ReadonlyMap<string, string>
  readonly commonSourceDirectory: string
}

const commonSourceDirectoryFor = (project: LoadedProject): string => {
  if (project.options.rootDir !== undefined) {
    return path.resolve(project.options.rootDir)
  }

  const sourceDirectories = project.fileNames
    .filter((fileName) => !/\.d\.tsx?$/u.test(fileName))
    .map((fileName) => path.resolve(path.dirname(fileName)))
  const [firstSourceDirectory] = sourceDirectories
  if (firstSourceDirectory === undefined) {
    throw new Error('api-lock: TypeScript project has no implementation source files.')
  }

  let commonDirectory = firstSourceDirectory
  while (
    !sourceDirectories.every(
      (directory) => directory === commonDirectory || directory.startsWith(`${commonDirectory}${path.sep}`),
    )
  ) {
...
export const emitDeclarations = (project: LoadedProject, outDir: string): EmittedDeclarations => {
  ...
  return { files: emitted, commonSourceDirectory: commonSourceDirectoryFor(project) }
}
...
/** Resolve an exported source entry point to TypeScript's declaration output. */
export const declarationPathFor = (
  entryPoint: string,
  outDir: string,
  commonSourceDirectory: string,
  repositoryRoot = rootDir,
): string => {
  const sourceFile = path.resolve(repositoryRoot, entryPoint)
  const relativeSourcePath = path.relative(commonSourceDirectory, sourceFile)
  if (
    relativeSourcePath === '' ||
    relativeSourcePath === '..' ||
    ...
    throw new Error(`api-lock: entry point ${JSON.stringify(entryPoint)} is outside the TypeScript source root.`)
  }

  return normalizePath(path.join(outDir, relativeSourcePath.replace(/\.tsx?$/u, '.d.ts')))
}
...
export const generate = async (): Promise<string> => {
  ...
  const virtualFiles = new Set(emitted.files.keys())
  const entryDeclarations = REPOSITORY_POLICY.entryPoints.map((entry) =>
    declarationPathFor(entry, outDir, emitted.commonSourceDirectory),
  )
  ...
  const host = createVirtualHost(emitted.files, dtsOptions)
